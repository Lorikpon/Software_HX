...
namespace Verwaltung_HomExtra
{
    public partial class Debitoren : Form
    {
        ...
        private void ApplyFilter()
        {
            ...
            // Beispiel: Filtern nach Checkboxen
            if (chkPLZ.Checked)
            {
                filter += "PLZ = '1100'"; // Beispielbedingung für die Filterung
            }
            //     if (checkBox2.Checked)
            {
                //         if (!string.IsNullOrEmpty(filter))
                //             filter += " AND ";
                //         filter += "Spalte2 = 'Bedingung2'"; // Beispielbedingung für die Filterung
            }

            // Filter anwenden, wenn eine Bedingung existiert
            if (!string.IsNullOrEmpty(filter))
            {
                (dgvDebitoren.DataSource as DataTable).DefaultView.RowFilter = filter;
            }
            else
            {
                (dgvDebitoren.DataSource as DataTable).DefaultView.RowFilter = string.Empty; // Alle Daten anzeigen, wenn kein Filter aktiv ist
            }
            con.Close();
        }
    }
